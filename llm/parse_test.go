...
// categoriasDePrueba es el conjunto CERRADO que el caller le ofrece al modelo.
// El enum no lo fija este paquete: design.md §3.2 del Plan 044 solo deja escrito
// que el score continuo no funciona y las categorías sí, sin nombrarlas.
func categoriasDePrueba() []string {
	return []string{"intake_request", "otra_cosa", "ruido"}
}

func TestParseClassification_SinScoreContinuo(t *testing.T) {
	out, err := llm.ParseClassification(json.RawMessage(
		`{"version": 1, "category": "intake_request", "evidence": "quiero pedir dos tortas"}`),
		categoriasDePrueba())
	require.NoError(t, err)
	assert.Equal(t, "intake_request", out.Category)
}

func TestParseClassification_CategoriaFueraDelEnumEsErrorDeCalidad(t *testing.T) {
	...
	fuera := []string{
		`{"version": 1, "category": "intake_requests", "evidence": "quiero dos tortas"}`,
		`{"version": 1, "category": "Intake_Request", "evidence": "quiero dos tortas"}`,
		`{"version": 1, "category": "", "evidence": "quiero dos tortas"}`,
		`{"version": 1, "evidence": "quiero dos tortas"}`,
	}
	for _, raw := range fuera {
		out, err := llm.ParseClassification(json.RawMessage(raw), categoriasDePrueba())
		requiereErrorDeCalidad(t, err)
		assert.Nil(t, out)
	}
}

func TestParseClassification_SinEnumDeclaradoNoPasaNada(t *testing.T) {
	// Un caller que no declara sus categorías es un cableado roto, y tiene que
	// fallar ruidoso: si aquí se dejara pasar «lo que sea» porque no hay con qué
	...
	// len(permitidos) == 0.
	out, err := llm.ParseClassification(json.RawMessage(
		`{"version": 1, "category": "intake_request", "evidence": "quiero dos tortas"}`), nil)
	requiereErrorDeCalidad(t, err)
	assert.Nil(t, out)
}
...
func ecoDelEsquemaDeClassify(t *testing.T) json.RawMessage {
	...
	prompt := llm.BuildClassifyRequestPrompt(llm.ClassifyRequestInput{
		Text:       "quiero pedir dos tortas",
		Categories: categoriasDePrueba(),
	})
	eco, err := llm.ExtractJSON(prompt)
...
func TestParseClassification_EcoDelEsquemaEsErrorDeCalidad(t *testing.T) {
	...
	// silencio, con Category valiendo el relleno del esquema, contaminando todo
	// lo que venga detrás sin que nadie vea un error.
	// Mutación que lo pone rojo: en ParseClassification, borrar la llamada a
	// validarEnum (o hacer que validarEnum devuelva siempre nil).
	out, err := llm.ParseClassification(ecoDelEsquemaDeClassify(t), categoriasDePrueba())
	requiereErrorDeCalidad(t, err)
	assert.Nil(t, out)
}
